...
hooks.Filters.CONFIG_DEFAULTS.add_items(
    [
        ...
        (
            ...
            [
                "git+https://github.com/eduNEXT/xblock-limesurvey.git@v0.5.1",
            ],
        ),
    ]
)
...
# tutorlimesurvey/templates/limesurvey/jobs/init/
...
for service, template_path in MY_INIT_TASKS:
    full_path: str = importlib.resources.files("tutorlimesurvey") / os.path.join("templates", *template_path)
...
hooks.Filters.ENV_TEMPLATE_ROOTS.add_items(
    ...
    [
        importlib.resources.files("tutorlimesurvey") / "templates",
    ]
)
...
hooks.Filters.ENV_TEMPLATE_TARGETS.add_items(
    ...
    # For example, ``tutorlimesurvey/templates/limesurvey/build``
    # will be rendered to ``$(tutor config printroot)/env/plugins/limesurvey/build``.
    [
        ("limesurvey/build", "plugins"),
        ("limesurvey/apps", "plugins"),
    ],
)
...
# For each file in tutorlimesurvey/patches,
# apply a patch based on the file's name and contents.
for path in glob(
    os.path.join(
        importlib.resources.files("tutorlimesurvey") / "patches",
        "*",
    )
):
...
### hooks.Filters.CLI_COMMANDS.add_item(limesurvey)
...
#   $ tutor limesurvey example-command
